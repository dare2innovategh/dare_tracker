...
import { Loader2, BarChart2, Plus, CalendarIcon, AlertTriangle } from "lucide-react";
...
// Custom NumberInput component to properly handle form values
type NumberInputProps = React.InputHTMLAttributes<HTMLInputElement> & {
  onValueChange: (value: number | undefined) => void;
  value: number | undefined;
};

function NumberInput({ onValueChange, value, ...props }: NumberInputProps) {
  return (
    <Input
      type="number"
      ...
      onChange={(e) => {
        ...
        const newValue = rawValue === "" ? undefined : Number(rawValue);
...
interface BusinessTrackingFormProps {
  ...
  existingData?: any; // The tracking record to edit
  isEditMode?: boolean;
}

export function BusinessTrackingFormFixed({
  business,
  currentUser,
  onSuccess,
  ...
  // Check if user has permission to create/edit business tracking
  ...
  // Determine if user should have access to the form
  // Admin has access to all features by default
  const hasAccess = isAdmin || (isEditMode ? canEditTracking : canCreateTracking);
  // We specifically don't want mentors to add tracking records
  const noAccessReasonDueToRole = isMentor ? "Mentors are not permitted to add business tracking records." : "";

  const today = new Date();
  const currentMonth = new Date(today.getFullYear(), today.getMonth(), 1);

  // Prepare default values based on whether we're in edit mode
  const getDefaultValues = () => {
    // If in edit mode and we have existing data, use it as defaults
    if (isEditMode && existingData) {
      // Parse dates from the existing data
      const trackingDate = existingData.trackingDate ? new Date(existingData.trackingDate) : today;
      const trackingMonth = existingData.trackingMonth ? new Date(existingData.trackingMonth) : currentMonth;
      ...
      return {
        ...
        trackingDate,
        trackingMonth,
        trackingYear: trackingMonth.getFullYear(),
        
        // Fill in existing data
        projectedRevenue: existingData.projectedRevenue,
        actualEmployees: existingData.actualEmployees,
        newEmployees: existingData.newEmployees,
        actualRevenue: existingData.actualRevenue,
        internalRevenue: existingData.internalRevenue,
        externalRevenue: existingData.externalRevenue,
        actualExpenditure: existingData.actualExpenditure,
        actualProfit: existingData.actualProfit,
        prominentMarket: existingData.prominentMarket || "",
        newResources: existingData.newResources || [],
        allEquipment: existingData.allEquipment || [],
        keyDecisions: existingData.keyDecisions || [],
        lessonsGained: existingData.lessonsGained || [],
        businessInsights: existingData.businessInsights || "",
        challenges: existingData.challenges || [],
        nextStepsPlanned: existingData.nextStepsPlanned || [],
        mentorFeedback: existingData.mentorFeedback || "",
      };
    }
  ...
  const trackingMutation = useMutation({
    mutationFn: async (values: AddTrackingFormValues) => {
      // We don't need to format the dates - we'll send the actual Date objects
      const payload = {
        ...values,
        // Send the Date objects directly
      ...
      const url = isEditMode 
        ? `/api/business-tracking/${existingData.id}` 
      ...
      if (!response.ok) {
        ...
        throw new Error(error.message || `Failed to ${isEditMode ? "update" : "add"} tracking record`);
      }
    ...
    onSuccess: () => {
      toast({
        title: isEditMode ? "Business Data Updated" : "Business Data Added",
        description: isEditMode 
          ? "Business data record has been updated successfully."
          : "Business data record has been added successfully.",
        variant: "default",
      });
      queryClient.invalidateQueries({ queryKey: [`/api/business-tracking/businesses/${business.id}/tracking`] });
      if (!isEditMode) form.reset(); // Only reset form for new records
      if (onSuccess) onSuccess();
    },
    onError: (error: Error) => {
      toast({
        title: "Error",
        description: error.message || `Failed to ${isEditMode ? "update" : "add"} business data`,
        variant: "destructive",
      });
    },
  });
  ...
  return (
    <Form {...form}>
      {/* If the user doesn't have proper permissions, show a warning message instead of the form */}
      {(isMentor || !hasAccess) ? (
        <Alert variant="destructive" className="mb-6">
          ...
          <AlertDescription>
            {isMentor 
              ? noAccessReasonDueToRole 
              : `You don't have permission to ${isEditMode ? 'edit' : 'create'} business tracking records.`}
          </AlertDescription>
        </Alert>
      ) : (
        ...
        <div className="flex flex-col sm:flex-row justify-between pt-6 border-t gap-3">
          ...
          <Button 
            ...
            className="bg-gradient-to-r from-[#FF5F00] via-[#EB001B] to-[#F79E1B] border-none text-white px-6 w-full sm:w-auto"
          >
            {trackingMutation.isPending ? (
              ...
              <>
                ...
                {isEditMode ? "Update Business Data" : "Add Business Data"}
              </>
            )}
          </Button>
        </div>
