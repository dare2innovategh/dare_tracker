...
import { Trash2, Edit2, Plus, Award, FileText } from "lucide-react";
...
export default function CertificationManager({ youthId }: CertificationManagerProps) {
  ...
  const editCertificateMutation = useMutation({
    mutationFn: async (data: CertificateFormValues) => {
      ...
      if (!response.ok) {
        console.error("Failed to update certificate:", await response.text());
        const errorData = await response.json().catch(() => ({ message: "Unknown error" }));
  ...
  const deleteCertificateMutation = useMutation({
    mutationFn: async () => {
      ...
      if (!response.ok) {
        console.error("Failed to delete certificate:", await response.text());
        const errorData = await response.json().catch(() => ({ message: "Unknown error" }));
  ...
  const handleEditClick = (certificate: any) => {
    ...
    const formValues = {
      ...
      trainingProgram: certificate.trainingProgram || "none",
  ...
  const getTrainingProgramName = (programId: string | null | undefined) => {
    if (!programId || !trainingPrograms || programId === "none") return "N/A";
    const program = trainingPrograms.find((p: any) => 
      p.id.toString() === programId || 
      p.name === programId ||
      p.id === parseInt(programId)
    );
    ...
    <div className="space-y-4">
              ...
              <CardContent className="py-2">
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-3 text-sm">
                  <div>
                    ...
                    <p>{certificate.trainingProgram ? getTrainingProgramName(certificate.trainingProgram) : "N/A"}</p>
                  </div>
                  <div>
            ...
            <form onSubmit={editForm.handleSubmit((data) => editCertificateMutation.mutate(data))} className="space-y-4">
              ...
              <div className="space-y-2">
                ...
                <Input
                  ...
                  onChange={handleFileChange}
                />
                {selectedCertificate?.credentialUrl && !removeFile && (
                  <div className="flex items-center mt-2">
                    ...
                    />
                    <label htmlFor="removeFile">Remove current file</label>
                  </div>
                )}
              </div>
      ...
      <Dialog open={isDeleteDialogOpen} onOpenChange={setIsDeleteDialogOpen}>
        <DialogContent>
          ...
          <p>
            Are you sure you want to delete the certificate "{selectedCertificate?.certificationName}"? This action cannot be undone.
          </p>
          <DialogFooter>
            <Button
              type="button"
